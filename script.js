...
// ゲームリセット
function resetGame() {
  gameState.currentPlayer = gameState.customRules.firstPlayer || 'p1';
  ...
  // ログクリア
  const startPlayerName = gameState.currentPlayer === 'p1' ? 'プレイヤー1' : p2Name.textContent;
  logList.innerHTML = `<div class="log-item system">ゲームが開始されました。最初のターンは ${startPlayerName} です。</div>`;
  
  updateUI();
  updateTurnIndicator();
  enablePlayerDrag();
  addLog('システム', `${startPlayerName}のターンです。`, 'system');
...
function executeAttack(attackerPlayerId, targetPlayerId, attackerCardId, targetCardId) {
  ...
  if (gameState.customRules.multiplyAttack) {
    nextTargetVal = attackerVal * targetVal;
  } else {
    nextTargetVal = attackerVal + targetVal;
  }
...
function endTurn() {
  ...
  if (gameState.customRules.chainExplosion) {
...
function checkVictory() {
  ...
  if (p1Defeated || p2Defeated || p1WonBySpecial || p2WonBySpecial) {
    ...
    if (gameState.campaignStage !== null) {
      ...
      btnBackToSelect.classList.remove('hidden');
    } else {
      btnNextStage.classList.add('hidden');
      btnRematch.classList.remove('hidden');
      btnBackToSelect.classList.add('hidden');
    }
...
// 手（カード）の名称取得
function getHandName(cardId) {
  const names = { 'A': '右手', 'B': '左手', 'C': '第3の手', 'D': '第4の手' };
  return names[cardId] || `カード${cardId}`;
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    simulateCalculateVal,
    simulateChainExplosion
  };
}
